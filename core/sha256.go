...
func expandMessage(block []byte) [64]uint32 {
...
func SHA256(input []byte, iterations int, rounding bool) [32]byte {
	// IV-Values
	...
	for n := 0; n < len(paddedMessage); n += 64 {
		...
		w := expandMessage(block)
		hashes = compress(hashes, w, rounding, iterations)
